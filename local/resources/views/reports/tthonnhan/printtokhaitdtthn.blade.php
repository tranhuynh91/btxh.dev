...
<table width="95%" border="0" cellspacing="0" cellpadding="8" style="border-collapse: collapse; margin:20px auto">
    ...
    <tr>
        <td colspan="2">Đã đăng ký: {{$model->plgiayto}} ngày {{date('d',strtotime($tthn->ngayxn))}} tháng {{date('m',strtotime($tthn->ngayxn))}} năm  {{date('Y',strtotime($tthn->ngayxn))}}. tại số: {{$tthn->so}}</td>
    </tr>
    <tr>
        <td colspan="2">Quyển số: {{$tthn->quyen}} của {{$tencq}}</td>
    </tr>
    <tr>
