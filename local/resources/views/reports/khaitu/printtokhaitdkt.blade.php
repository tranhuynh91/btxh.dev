...
<table width="95%" border="0" cellspacing="0" cellpadding="8" style="border-collapse: collapse; margin:20px auto">
    ...
    <tr>
        <td colspan="2">Đã đăng ký: {{$model->plgiayto}} ngày {{date('d',strtotime($khaitu->ngaydangkykt))}} tháng {{date('m',strtotime($khaitu->ngaydangkykt))}} năm  {{date('Y',strtotime($khaitu->ngaydangkykt))}}. tại số: {{$khaitu->so}}</td>
    </tr>
    <tr>
        <td colspan="2">Quyển số: {{$khaitu->quyen}} của {{$tencq}}</td>
    </tr>
    <tr>
