...
<table width="95%" border="0" cellspacing="0" cellpadding="8" style="border-collapse: collapse; margin:20px auto">
    ...
    <tr>
        <td colspan="2">Đã đăng ký: {{$model->plgiayto}} ngày {{date('d',strtotime($connuoi->ngaythangdk))}} tháng {{date('m',strtotime($connuoi->ngaythangdk))}} năm  {{date('Y',strtotime($connuoi->ngaythangdk))}}. tại số: {{$connuoi->so}}</td>
    </tr>
    <tr>
        <td colspan="2">Quyển số: {{$connuoi->quyen}} của {{$tencq}}</td>
    </tr>
    <tr>
