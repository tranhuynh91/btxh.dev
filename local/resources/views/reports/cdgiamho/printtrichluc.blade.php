...
<html>
<head>
    <title>In trích lục đăng ký chấm dứt giám hộ (Bản sao)</title>
...
<table width="95%" border="0" cellspacing="0" cellpadding="8" style="border-collapse: collapse; margin: auto;text-align: center">
    <tr>
        <td>Số {{$modeltt->socd}}/TLCDGH-BS</td>
        <td>@if($xa=="Thị Trấn Yên Minh"){{$xa}}
            @elseif(session('admin')->name=="Phòng Tư Pháp Huyện Yên Minh") {{"Phòng Tư Pháp Huyện Yên Minh"}}
            @elseif(session('admin')->name=="Phòng Tư Pháp Huyện Đồng Văn") {{"Phòng Tư Pháp Huyện Đồng Văn"}}
            @else{{$tenxa}}@endif, ngày {{date('d',strtotime($model->ngaycap))}} tháng {{date('m',strtotime($model->ngaycap))}} năm  {{date('Y',strtotime($model->ngaycap))}}
        </td>
    </tr>
</table>
...
<table width="95%" border="0" cellspacing="0" cellpadding="8" style="border-collapse: collapse; margin:20px auto">
    ...
    <tr>
        <td colspan="3"><b>Đã đăng ký chấm dứt giám hộ,</b> số đăng ký {{$modeltt->socd}} ngày {{getDayVn($modeltt->ngaychamdut)}}</td>
    </tr>
    <tr>
        <td colspan="3"><b>Ghi chú :</b> {{$modeltt->ghichu}}</td>
    </tr>
</table>
