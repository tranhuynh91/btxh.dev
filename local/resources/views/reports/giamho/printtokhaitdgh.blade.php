...
<table width="95%" border="0" cellspacing="0" cellpadding="8" style="border-collapse: collapse; margin:20px auto">
    ...
    @if($giamho->phanloainhap == "Giám hộ")
        <tr>
            <td colspan="2">Đã đăng ký: {{$model->plgiayto}} ngày {{date('d',strtotime($giamho->ngaydangky))}} tháng {{date('m',strtotime($giamho->ngaydangky))}} năm  {{date('Y',strtotime($giamho->ngaydangky))}}. tại số: {{$giamho->soso}}</td>
        </tr>
        <tr>
            <td colspan="2">Quyển số: {{$giamho->soquyen}} của {{$tencq}}</td>
        </tr>
    @elseif($giamho->phanloainhap == "Chấm dứt giám hộ")
        <tr>
            <td colspan="2">Đã đăng ký: {{$model->plgiayto}} ngày {{date('d',strtotime($giamho->ngaydangky))}} tháng {{date('m',strtotime($giamho->ngaydangky))}} năm  {{date('Y',strtotime($giamho->ngaydangky))}}. tại số: {{$giamho->socd}}</td>
        </tr>
        <tr>
            <td colspan="2">Quyển số: {{$giamho->quyencd}} của {{$tencq}}</td>
        </tr>
    @endif
    <tr>
