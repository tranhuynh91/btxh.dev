...
<table width="95%" border="0" cellspacing="0" cellpadding="8" style="border-collapse: collapse; margin:20px auto">
    ...
    <tr>
        <td colspan="2">Đã đăng ký: {{$model->plgiayto}} ngày {{date('d',strtotime($cmc->ngaydangky))}} tháng {{date('m',strtotime($cmc->ngaydangky))}} năm  {{date('Y',strtotime($cmc->ngaydangky))}}. tại số: {{$cmc->soso}}</td>
    </tr>
    <tr>
        <td colspan="2">Quyển số: {{$cmc->soquyen}} của {{$tencq}}</td>
    </tr>
    <tr>
