...
<table width="95%" border="0" cellspacing="0" cellpadding="8" style="border-collapse: collapse; margin:20px auto">
    ...
    <tr>
        <td colspan="2">Đã đăng ký: {{$model->plgiayto}} ngày {{date('d',strtotime($kethon->ngaydangky))}} tháng {{date('m',strtotime($kethon->ngaydangky))}} năm  {{date('Y',strtotime($kethon->ngaydangky))}}. tại số: {{$kethon->sokethon}}</td>
    </tr>
    <tr>
        <td colspan="2">Quyển số: {{$kethon->quyenkethon}} của {{$tencq}}</td>
    </tr>
    <tr>
